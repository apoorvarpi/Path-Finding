...
int main(int argc, char **argv)
{
  ...
  while (Aria::getRunning()) 
  {
    ...
    cout<<"\nForward Theta = "<<thetag<<" Pulling Back Theta = "<<thetag1<<"Robot Theta = "<< robot.getTh()<<"\n";
    if(distg>3.2){
	cout<<distg<<" Child Pulling Back\n";   
    	Frx+=100*(att*cos(thetag1*PI/180)*distg);
    	Fry+=100*(att*sin(thetag1*PI/180)*distg);
	Frx1+=100*(att*cos(thetag1*PI/180)*distg);
    	Fry1+=100*(att*sin(thetag1*PI/180)*distg);
    }
    else{
	Frx-=(rep*cos(thetag1*PI/180)/distg);
    	Fry-=(rep*sin(thetag1*PI/180)/distg);
    }
    cout<<"\nFlag = "<<flag<<"\n";

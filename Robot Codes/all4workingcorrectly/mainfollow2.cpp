...
int main(int argc, char **argv)
{
  ...
  while (Aria::getRunning()) 
  {
    ...
    cx=robot.getX();
    cy=robot.getY();
    ofstream newFile("position4.txt");
    ...
    newFile.close();
    newFile.open("/home/rahul/Path-Finding/A-star/position4.txt");
    if(newFile.is_open())   
    {
        newFile << cx << " " << cy<<"\n";            
    }
    newFile.close();    
     ...
     myReadFile1.open("position3.txt");
    ...
    cout<<"Distance :"<<distg<<"\n";
    ofstream newFile1("distance34.txt");
    ...
    newFile1.close();
    double t1;
    myReadFile2.open("distance45.txt");
	...
	if(obsr[i]!=0){
		...
		if((obsr[i]*100)<390){
	                Frx-=(1.2*(rep*cos(obsang[i] * PI / 180.0))/(obsr[i]*obsr[i]));
		    	Fry-=(1.2*(rep*sin(obsang[i] * PI / 180.0))/(obsr[i]*obsr[i]));
			flag=1;
		}
	}
    }
